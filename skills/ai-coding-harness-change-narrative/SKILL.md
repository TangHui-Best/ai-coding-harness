---
name: ai-coding-harness-change-narrative
...
ai-coding-harness-knowledge-capture owns structured project memory.
```
...
- A way to bypass `ai-coding-harness-knowledge-capture` when structured Harness artifacts are triggered.
...
- Do not re-enter `ai-coding-harness-knowledge-capture` after it has produced a `pass` or permitted `conditional` verdict for the same closeout. Link or summarize that verdict instead.
...
3. For non-trivial bug fixes, inspect history around touched files and link the Feature attribution result from `ai-coding-harness-knowledge-retrieval` when available. Identify when behavior likely changed, the earlier intent, and whether the new fix preserves or revises that intent.
...
7. If durable project memory is triggered, hand off to `ai-coding-harness-knowledge-capture`.
