...
description: MUST use as the AI Coding Harness entrypoint before non-trivial engineering work, behavior changes, reviews, commits, PRs, handoffs, or any completion claim; also use when the user mentions Harness, gates, Evidence, ADRs, Lessons, Feature memory, patch churn, 知识沉淀, 收尾, 完成声明, 提交信息, or PR 描述.
---
...
Use this skill as the lightweight router for AI Coding Harness. Keep it in memory only long enough to decide which specific AI Coding Harness skill, reference, or script is needed.
...
The formal system name is `AI Coding Harness`. Use the full name when introducing the system, writing durable project memory, or distinguishing it from a project's own test harness, runtime harness, evaluation harness, or business feature named "harness".

`Harness` is only a short name after `AI Coding Harness` has been defined in the current context.

The installed skill slugs are `ai-coding-harness` and `ai-coding-harness-*`. Pre-rename public skill slugs were removed by the breaking rename. Do not create bare workflow skills outside the `ai-coding-harness` namespace unless a future ADR explicitly accepts another migration path.

When the user or repository mentions "harness" ambiguously, distinguish project-internal harness code from AI Coding Harness gates before acting.
...
2. For non-trivial work, run `ai-coding-harness-start-gate` before implementation.
3. For completion/readiness claims, route to `ai-coding-harness-knowledge-capture`.
4. For commit, PR, release, handoff, progress, or rejected-path narrative, route to `ai-coding-harness-change-narrative`.
...
- Entry Gate: run `ai-coding-harness-start-gate` before implementation and satisfy any required retrieval, Vision Anchor, Feature, spec, plan, ADR, or delegation decision before coding.
- Exit Gate: run `ai-coding-harness-knowledge-capture` before saying the work is complete, fixed, verified, ready for PR, ready for review, ready for handoff, safely closed, or mergeable.
...
## AI Coding Harness Presence Check
...
1. `ai-coding-harness-start-gate` before non-trivial implementation.
2. `ai-coding-harness-knowledge-retrieval` when prior Feature, ADR, Lesson, spec, plan, Evidence, or bug attribution may change the fix.
3. `ai-coding-harness-doc-lifecycle` for stale, archived, superseded, deprecated, invalidated, or replaced documents.
4. `ai-coding-harness-incident-learning` after a fixed or stabilized bug, incident, recurring failure, or patch chain needs prevention analysis.
5. `ai-coding-harness-delegation-gate` before work that may need subagents or independent review.
6. `ai-coding-harness-vision-gate` when intent, scope, product direction, or acceptance may drift.
7. `ai-coding-harness-readiness-dashboard` for review, release, handoff, or readiness rollups.
8. `ai-coding-harness-change-narrative` for commit messages, PR descriptions, release notes, handoff notes, progress summaries, root cause, rejected paths, and history-aware change explanations.
9. `ai-coding-harness-knowledge-capture` for Evidence, closeout, durable memory, completion verdict, and completion-claim permission.
10. `ai-coding-harness-project-rules` before promoting a decision, Lesson, or repeated constraint into `AGENTS.md` or another project-level rule file.
...
- Do not re-enter `ai-coding-harness` for the same task transition after it has already routed to a more specific AI Coding Harness skill.
- `ai-coding-harness-knowledge-capture` owns the final completion verdict. When it returns `pass`, or an explicitly permitted `conditional` verdict for the requested stage, the closeout is terminal.
...
- Do not route a normal final response to `ai-coding-harness-change-narrative`. Use change narrative for commit messages, PR descriptions, handoff notes, release notes, progress summaries, rejected-path explanations, and history-aware engineering narratives.
- If a specific AI Coding Harness skill reports blocked or conditional status, preserve that status directly instead of re-routing through this entrypoint to look for a different answer.

## Reference Map

Use references only when their trigger applies.

- `references/routing.md`: read when routing is ambiguous, multiple AI Coding Harness skills appear to apply, or trigger wording needs detail.
...
For this repository, prefer `scripts/install.ps1 codex` or `scripts/install.sh codex` to sync AI Coding Harness skills into the local Codex skills directory instead of hand-copying individual files.
...
- Do not load every AI Coding Harness skill preemptively.
- Do not read bundled scripts just to learn how to run them.
- Do not create documents for every small change.
- Do not edit `AGENTS.md` just because Harness memory exists; route through `ai-coding-harness-project-rules`.
...
| "No formal artifact is needed, so no AI Coding Harness skill is needed." | AI Coding Harness may conclude no artifact is needed; the check is still the gate. |
