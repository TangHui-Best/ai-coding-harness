---
name: ai-coding-harness-incident-learning
...
- A change-summary workflow. Use `ai-coding-harness-change-narrative`.
- A formal artifact writer. Use `ai-coding-harness-knowledge-capture`.
...
2. Confirm the Feature attribution from `ai-coding-harness-knowledge-retrieval` when the bug is not tiny. If attribution was skipped, explain why project memory could not change the learning.
...
7. Route durable knowledge only when useful: narrative to `ai-coding-harness-change-narrative`; Lesson, ADR, Evidence, Feature, or Backlog records to `ai-coding-harness-knowledge-capture`.
...
5. If the answer is no, route to `ai-coding-harness-vision-gate` and consider ADR or Lesson before more implementation.

If `## Patch History` reaches 3 rows, route to `ai-coding-harness-knowledge-capture` to ensure a non-empty `## Patch Churn Review` is recorded before the next patch is closed.
...
- Did the final response use completion/readiness wording without the closeout categories required by `ai-coding-harness-knowledge-capture`?
...
Scale Evidence to risk. Low-risk documentation work can report Evidence in the final response. Medium/high-risk incidents should route to `ai-coding-harness-knowledge-capture` for a durable Evidence record.

## Routing

Use `ai-coding-harness-change-narrative` when the root cause, trigger, fix rationale, rejected approaches, or PR/commit/handoff summary needs to be explained.

Use `ai-coding-harness-knowledge-capture` when the learning should become a Lesson, ADR, Evidence, Feature link, Backlog item, or another durable Harness artifact.
...
| Creating artifacts directly. | Decide and route to `ai-coding-harness-knowledge-capture`. |
