...
Use this skill to decide whether AgentMentor documents should stay visible in active knowledge paths, move toward archive, or be linked as replaced or updated.
...
This skill governs lifecycle metadata and placement only. If the lifecycle change also changes durable project knowledge, route to `knowledge-capture`.
...
- Stale, superseded, deprecated, completed, or archived AgentMentor documents.
- Choosing between `invalidates`, `updates`, and `superseded_by`.

Do not use for:

- Writing all knowledge capture from scratch. Use `knowledge-capture`.
...
Route to `knowledge-capture` when a lifecycle change affects any durable knowledge entry:
