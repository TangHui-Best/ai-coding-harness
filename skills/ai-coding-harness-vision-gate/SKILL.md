---
name: ai-coding-harness-vision-gate
...
- Deciding whether a Feature, spec, plan, or ADR must exist before coding. Use `ai-coding-harness-start-gate`.
- Writing official ADR, Lesson, Evidence, or Feature updates. Route to `ai-coding-harness-knowledge-capture`.
- Explaining why implementation paths were rejected. Route to `ai-coding-harness-change-narrative`.
...
- Existing Harness context that may affect intent, after `ai-coding-harness-knowledge-retrieval` when needed.
...
Use `ai-coding-harness-delegation-gate` in `review` mode when independent review may reduce risk, or when platform policy requires user permission before dispatching an independent reviewer.
...
If an independent reviewer is needed but unavailable, say so and route the residual risk to `ai-coding-harness-readiness-dashboard` or `ai-coding-harness-knowledge-capture` instead of silently self-approving.
...
Lenses are aids, not new sources of truth. If a lens finds a recurring issue, route the learning through `ai-coding-harness-knowledge-capture`.
...
| `needs knowledge capture` | A decision, lesson, evidence record, or Feature state needs durable memory. | Use `ai-coding-harness-knowledge-capture`. |

If the gate exposes a rejected path that must be explained for reviewers or future maintainers, use `ai-coding-harness-change-narrative`.
...
| Writing ADR/Lesson/Evidence here. | Route durable knowledge work to `ai-coding-harness-knowledge-capture`. |
