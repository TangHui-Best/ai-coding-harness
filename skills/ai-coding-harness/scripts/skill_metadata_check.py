#!/usr/bin/env python3
"""Validate AI Coding Harness skill metadata and trigger-surface health."""
...
RECOMMENDED_WORKFLOW_PREFIX = "ai-coding-harness-"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Validate AI Coding Harness skill frontmatter, encoding, and trigger metadata."
    )
...
def validate_skill(path: Path) -> list[Issue]:
    ...
    if name == "ai-coding-harness":
        ...
        for heading in ["## Activation Contract", "## AI Coding Harness Presence Check"]:
            ...
            if not resource_path.exists():
                issues.append(
                    Issue(
                        "error",
                        resource_path,
                        "ai-coding-harness is missing required bundled resource.",
                    )
                )
        ...
        issues.append(
            Issue(
                ...
                "use `ai-coding-harness` or `ai-coding-harness-*`.",
            )
        )
    elif name.startswith(RECOMMENDED_WORKFLOW_PREFIX):
        if "MUST use" not in description:
            issues.append(
                Issue(
                    "warning",
                    path,
                    "AI Coding Harness workflow skill description should usually "
                    "start with an independent trigger.",
                )
            )
