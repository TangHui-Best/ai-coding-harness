...
- AgentMentor process misses, such as skipped closeout, missing Evidence level, missing `knowledge_check.py` after artifact edits, skipped Start/Vision/Readiness Gate, or completion language used while closeout was incomplete.
...
- A change-summary workflow. Use `change-narrative`.
- A formal artifact writer. Use `knowledge-capture`.
...
2. Confirm the Feature attribution from `knowledge-retrieval` when the bug is not tiny. If attribution was skipped, explain why project memory could not change the learning.
...
7. Route durable knowledge only when useful: narrative to `change-narrative`; Lesson, ADR, Evidence, Feature, or Backlog records to `knowledge-capture`.
...
If `## Patch History` reaches 3 rows, route to `knowledge-capture` to ensure a non-empty `## Patch Churn Review` is recorded before the next patch is closed.

For AgentMentor process misses, also identify:
...
- Did a AgentMentor artifact exist while Exit Gate status, Evidence level, or check result was still missing?
- Did the final response use completion/readiness wording without the closeout categories required by `knowledge-capture`?
...
Scale Evidence to risk. Low-risk documentation work can report Evidence in the final response. Medium/high-risk incidents should route to `knowledge-capture` for a durable Evidence record.

## Routing

Use `change-narrative` when the root cause, trigger, fix rationale, rejected approaches, or PR/commit/handoff summary needs to be explained.

Use `knowledge-capture` when the learning should become a Lesson, ADR, Evidence, Feature link, Backlog item, or another durable AgentMentor artifact.
...
| Creating artifacts directly. | Decide and route to `knowledge-capture`. |
