...
- `ai-coding-harness`: triggered only if Harness is mentioned or repo policy requires it.
- `ai-coding-harness-start-gate`: may return `ready` with `Task class: tiny`.
- `ai-coding-harness-knowledge-retrieval`: not triggered because project memory cannot change the fix.
...
- `ai-coding-harness-start-gate`: return `needs retrieval` when ownership or prior history is unknown.
- `ai-coding-harness-knowledge-retrieval`: establish Feature attribution before code search or edits.
- `ai-coding-harness-knowledge-capture`: record Evidence and Bugfix attribution; update Patch History when an existing completed Feature owns the behavior.
...
- `ai-coding-harness-change-narrative`: use for commit message and compact change rationale.
- `ai-coding-harness-knowledge-capture`: use only if durable memory, Evidence, or completion readiness is still missing.
...
- `ai-coding-harness-change-narrative`: owns PR narrative.
- `ai-coding-harness-readiness-dashboard`: use when readiness or blockers need a concise rollup.
- `ai-coding-harness-knowledge-capture`: use if completion, Evidence, or durable artifact state must be claimed.
...
- `ai-coding-harness-knowledge-capture`: record Evidence level and closeout state.
