...
| Readiness | For non-trivial review, release, handoff, or readiness claims, use `readiness-dashboard` or record why it was not triggered. |
| Vision Gate Exit | For non-trivial, user-facing, architecture, scope-sensitive, or behavior-changing work, use `vision-gate` in Exit Gate mode or record why it was not triggered. |
...
- `Check` records the actual `knowledge_check.py` result when AgentMentor artifacts were created or updated.
