...
- `using-agentmentor`: triggered only if Harness is mentioned or repo policy requires it.
- `start-gate`: may return `ready` with `Task class: tiny`.
- `knowledge-retrieval`: not triggered because project memory cannot change the fix.
...
- `start-gate`: return `needs retrieval` when ownership or prior history is unknown.
- `knowledge-retrieval`: establish Feature attribution before code search or edits.
- `knowledge-capture`: record Evidence and Bugfix attribution; update Patch History when an existing completed Feature owns the behavior.
...
- `change-narrative`: use for commit message and compact change rationale.
- `knowledge-capture`: use only if durable memory, Evidence, or completion readiness is still missing.
...
- `change-narrative`: owns PR narrative.
- `readiness-dashboard`: use when readiness or blockers need a concise rollup.
- `knowledge-capture`: use if completion, Evidence, or durable artifact state must be claimed.
...
Shape: Feature, ADR, Lesson, Evidence, Backlog, template, or AgentMentor docs changed.

Expected route:

- `knowledge-capture`: record Evidence level and closeout state.
- Run `knowledge_check.py` when AgentMentor knowledge artifacts changed.
