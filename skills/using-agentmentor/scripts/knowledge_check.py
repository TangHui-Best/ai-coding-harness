#!/usr/bin/env python3
"""Validate AgentMentor knowledge-capture Markdown artifacts."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Validate AgentMentor knowledge-capture Markdown artifacts."
    )
            ...
            if "knowledge_check" not in evidence.content:
                issues.append(
                    Issue(
                        "error",
                        evidence.path,
                        "Evidence for a completed feature must record AgentMentor validation "
                        "(`knowledge_check.py` command path and result) when AgentMentor "
                        "artifacts changed.",
                    )
                )
