...
| Readiness | For non-trivial review, release, handoff, or readiness claims, use `ai-coding-harness-readiness-dashboard` or record why it was not triggered. |
| Vision Gate Exit | For non-trivial, user-facing, architecture, scope-sensitive, or behavior-changing work, use `ai-coding-harness-vision-gate` in Exit Gate mode or record why it was not triggered. |
