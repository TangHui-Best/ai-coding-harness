...
- Deciding whether a Feature, spec, plan, or ADR must exist before coding. Use `start-gate`.
...
- Existing AgentMentor context that may affect intent, after `knowledge-retrieval` when needed.
...
Use `delegation-gate` in `review` mode when independent review may reduce risk, or when platform policy requires user permission before dispatching an independent reviewer.
...
If an independent reviewer is needed but unavailable, say so and route the residual risk to `readiness-dashboard` or `knowledge-capture` instead of silently self-approving.
...
Lenses are aids, not new sources of truth. If a lens finds a recurring issue, route the learning through `knowledge-capture`.
...
| `needs knowledge capture` | A decision, lesson, evidence record, or Feature state needs durable memory. | Use `knowledge-capture`. |

If the gate exposes a rejected path that must be explained for reviewers or future maintainers, use `change-narrative`.
...
| Writing ADR/Lesson/Evidence here. | Route durable knowledge work to `knowledge-capture`. |
