...
当前 Harness 已经有 `ai-coding-harness-delegation-gate`，但它的触发语义偏软：复杂任务只有在用户显式提到 subagent、parallel agents 或 independent review 时才稳定触发。对于长时间、跨模块、可拆分或高风险任务，这会让 Agent 默默退回单 Agent 工作，直到中途遇到授权问题才停住。
...
当用户描述长时间或 unattended 任务时，`ai-coding-harness` 必须提前路由到 Delegation Gate，在任务中途卡住前请求必要预授权。
...
- 把规则写进 `AGENTS.md`：暂不采用。本次目标是修复 AI Coding Harness skill 自身的触发约束；项目级规则可以作为后续建议，但不属于 skill 包本身。
