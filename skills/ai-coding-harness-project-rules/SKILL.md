---
name: ai-coding-harness-project-rules
...
The reusable project-rule starter template is bundled with the `ai-coding-harness` entrypoint skill at `ai-coding-harness/assets/templates/AGENTS.md`. Copy that template only when a project needs an `AGENTS.md` file or an existing file needs a small, source-backed Harness section.
...
This skill does not replace `ai-coding-harness-knowledge-capture`. Use it after a decision, lesson, evidence pattern, or recurring constraint may need to become a durable rule that every future agent should see before working in the project.
...
2. Check whether the source artifact already exists. If not, use `ai-coding-harness-knowledge-capture` first when rationale, recurrence, or evidence would otherwise be lost.
...
## Relationship To Other AI Coding Harness Skills

- Use `ai-coding-harness-change-narrative` first when the candidate rule comes from a specific change that needs its story distilled.
- Use `ai-coding-harness-incident-learning` first when the candidate rule comes from a bug, outage, regression, or repeated failure.
- Use `ai-coding-harness-knowledge-capture` first when the source should become an ADR, Lesson, Evidence, Feature, Backlog, or handoff artifact.
